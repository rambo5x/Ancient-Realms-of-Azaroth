...
public class QuestManager : MonoBehaviour
{
    ...
    {
        QuestObjectActivator[] questObjects = FindObjectsOfType<QuestObjectActivator>(); //how to find objects with a certain script
    ...
    public void SaveQuestData() //store basic data with player prefs if quest is complete or not
    {
        for(int i = 0; i < questMarkerNames.Length; i++)
        {
    ...
    public void LoadQuestData()
    {
        ...
        {
            int valueToSet = 0;
            if(PlayerPrefs.HasKey("QuestMarker_" + questMarkerNames[i])) //check if the quest exists in the saved data
            {
